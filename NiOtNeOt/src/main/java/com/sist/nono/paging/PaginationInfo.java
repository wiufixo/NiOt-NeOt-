...
//		/** 5. 총 게시글 수 **/
//		setTotalRecordCnt(totalRecordCnt);


		/** 6. 총 페이지 수 **/
		// 한 페이지의 최대 개수를 총 게시물 수 * 1.0로 나누어주고 올림 해준다.
		// 총 페이지 수 를 구할 수 있다.
		setTotalPageCnt((int) Math.ceil(totalRecordCnt / (double)pageSize));


		/** 7. 총 블럭 수 **/
		// 한 블럭의 최대 개수를 총  페이지의 수 * 1.0로 나누어주고 올림 해준다.
		// 총 블럭 수를 구할 수 있다.
		setTotalBlockCnt((int) Math.ceil(totalPageCnt / (double)blockSize));
		
        
		/** 4. 현재 블럭 **/
		// 현재 페이지 * 1.0을 블록의 최대 개수로 나누어주고 올림한다.
		// 현재 블록을 구할 수 있다.
		setBlock((int) Math.ceil(page/(double)blockSize)); 
		...
		/* 이전 페이지 존재 여부 */
		setHasPreviousPage(startPage != 1);
		
		/* 다음 페이지 존재 여부 */
		setHasNextPage((endPage * pageSize) < totalRecordCnt);
	}
